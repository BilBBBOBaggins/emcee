...
Если термины ниже незнакомы, короткий словарь есть в конце [README.ru.md](README.ru.md).
...
> на Codex читайте `AGENTS.md` (это ваш вход регламента). Подробно — [README](README.ru.md) →
...
1. **`/kickoff` — главный шаг.** Это команда, которая выводит проект из состояния «пусто» в «есть
   ...
   `docs/day-0-guide.md` и `docs/day-1-guide.md` (если генератор в `--mode new` уже создал
   `day-0-guide`, kickoff обновляет этот файл на месте, а не пишет второй). Вам не нужно вручную
   копаться в `{{...}}` — вы просто отвечаете на вопросы. (Подробно — [core/pipeline.md](core/pipeline.md).)
...
`day-0-guide` не создаётся, потому что проект уже инициализирован. `--backend` можно не указывать:
стек автоопределяется по маркер-файлам проекта (`pom.xml`/`build.gradle`, `composer.json`,
`go.mod`, `*.dproj`, …) и предлагается как дефолт; определённый вариант сборки/фреймворка
(Maven vs Gradle, Laravel vs Symfony) записывается в секцию `## Stack` энтри-файла.
...
5. **Коммит — ваш.** Агент выводит готовую команду, но **коммитите только вы** (по каждой задаче;
   единственное исключение — автономный прогон или гайд дня, явно назначивший коммит роли,
   [core/task-protocol.md](core/task-protocol.md)).
