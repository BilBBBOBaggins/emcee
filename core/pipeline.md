...
2. **Заполняет входной файл регламента** из ответов — стек, архитектура, команды сборки/тестов, — и
   **владеет `stack/<стек>.md`** (стек не из каталога → скелет из `_TEMPLATE.md`, дозаполнение —
   задача дня-0; не пользователь — [roles/architect.md](../roles/architect.md) §Kickoff). **Provenance-
