...
| **churn-cap barren-round counter** ([ADR-022](../docs/adr/022-obligations-governance-autonomous-profile.md)) | reviewer/architect + orchestrator | mechanical where a doctor/orchestrator check runs (new finding id + attached fix commit / red test, read from git artifacts) — planned doctor check; prose accountability meanwhile | **prose** — same degradation as hook gates (headless hooks don't fire); the carrier is the rule text in quality-gates.md |
