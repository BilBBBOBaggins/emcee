...
## Достижимость фичи в сборе — [QG-NN-05 · non-negotiable · accountability · ПРОВИЗОРНО до ратификации [ADR-015](../docs/adr/015-assembled-reachability-gate.md) панелью]
...
Правило: **каждый пункт замороженного объёма (frozen scope / acceptance) обязан иметь ≥1 прогон через
реальный composition root** — ту же точку входа, которой фичу собирает поставка (app entry / `useX` /
`startX` / DI-корень), **без bespoke-инъекции** обвязки, которую продукт обязан предоставить сам. Тест
доводит собранную систему до характерного случая фичи и ассертит, что фича **там наблюдаема**.

- **«В сборе» = через composition root, не обязательно через браузер.** Тестируй через реальную точку
  сборки; часть покрытия может идти в быстром раннере на уровне сборки, тонкий e2e — сверху. Запрет —
  не «медленно», а **инъекция интеграции, которую поставка опускает** (обобщение анти-паттерна
  qa-e2e #5 «invoke обходит UI» на любой слой: тест, вписывающий проводку сам, проверяет юнит, а не
  продукт).
- **Детерминизм — предусловие.** «В сборе через случайную игру» = флаки/бесконечно. Довод собранной
  системы до нужного состояния — через явную ручку (сеяный RNG, seed/config-override), не рандомом. Это
  тянет обязательный **control-surface детерминизма** в сам продукт.
- **Покрытие, не комбинаторика.** «Каждая фича в своём характерном случае», не «каждая комбинация» —
  пространство состояний взрывается. Цель: каждая frozen-фича достижима и наблюдаема, не полнота перебора.
...
действует и без развёрнутого QA. Пункт frozen scope без assembled-пути = задача **НЕ done** (под запись,
как QG-NN-04). В exit/QA-отчёте: по каждому пункту frozen scope — assembled-путь + ассерт наблюдаемости.
